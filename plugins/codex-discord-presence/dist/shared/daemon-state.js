...
function createDaemonStateManager(options) {
    ...
    function writeDaemonState(dataDir, state) {
        if (!isValidDaemonState(state))
            throw new Error('無效的常駐程序狀態。');
    ...
    function removeDaemonState(dataDir, expectedState) {
        ...
        if (!state)
            return false;
        if (expectedState && (state.pid !== expectedState.pid || state.instanceToken !== expectedState.instanceToken))
            return false;
    ...
    function isOwnedDaemon(state) {
        if (!isValidDaemonState(state) || !isRunning(state.pid))
            return false;
